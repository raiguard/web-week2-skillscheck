...
function bigOrSmall(arr) {
  let answers = [];
  arr.forEach((value) => {
    answers.push(value > 100 ? "big" : "small");
  });
  return answers;
}
...
const doubled = myNumbers.map((value) => value * 2);
...
const total = myNumbers.reduce((sum, value) => sum + value, 0);
...
myNumbers.forEach((_, index) => myNumbersIndex.push(index));
...
const totallyAwesome = people.reduce((sum, person) => sum + person.awesomeLevel, 0);
